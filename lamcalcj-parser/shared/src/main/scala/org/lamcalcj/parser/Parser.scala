...
object Parser {
  ...
  sealed case class Location(line: Int, column: Int) {
    ...
    final def advanceString(s: String): Location = s.foldLeft(this)(_ advanceChar _)

    final def advanceChar(c: Char): Location = c match {
      case '\n' => Location(line + 1, 1)
  ...
  sealed case class ParserError(location: Location, messages: List[Message]) {
    final def ++(e: ParserError): ParserError =
      if (messages.isEmpty && e.messages.nonEmpty)
        e
      else if (messages.nonEmpty && e.messages.isEmpty)
        this
    ...
    override def toString: String =
      "unexpected: (" + messages.filter(_.isInstanceOf[MessageUnexpected]).distinct.map(_.message).sorted.mkString(", ") + ")\n" +
        "excepted: (" + messages.filter(_.isInstanceOf[MessageExpected]).distinct.map(_.message).sorted.mkString(", ") + ")\n" +
        "message: (" + messages.filter(_.isInstanceOf[MessageRaw]).distinct.map(_.message).sorted.mkString(", ") + ")\n" +
        "at " + location
  }
}
